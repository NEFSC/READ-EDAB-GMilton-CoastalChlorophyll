...
Code for developing shapefile used in coastal_chlorophyll data
...
def combine_coastal_shapefiles(na_shapefile_path, usa_shapefile_path, output_path):
    ...
    gdf_na = gpd.read_file(na_shapefile_path)
    ...
    combined = pd.concat([gdf_na[['geometry']], gdf_usa[['geometry']]])
    combined['merge_id'] = 1
    dissolved_result = combined.dissolve(by='merge_id')
    dissolved_result.to_file(output_path)
